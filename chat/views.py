        ...
        try:
            ...
            for room in rooms:
                ...
                if request.user.is_authenticated and room.created_by:
                    ...
                    print(
                        f"🗑️ 방 '{room.name}': can_delete={can_delete} (생성자: {room.created_by.username}, 현재: {request.user.username})"
                    )
            ...
            print(
                f"✅ 총 {len(rooms_data)}개 방 반환, 인증상태: {request.user.is_authenticated}"
            )
...
class RoomCreateAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print(
                f"✅ 방 생성 완료: {room.name}, 생성자: {room.created_by.username}, 최대인원: {max_members}"
            )
